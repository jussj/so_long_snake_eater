...
/*   Updated: 2021/11/24 15:48:37 by jusaint-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	render_map(t_data *data)
{
	...
	while (data->scene->map[i])
	{
		...
		while (data->scene->map[i][j])
		{
			...
			if (data->scene->map[i][j] == EMPTY
				|| data->scene->map[i][j] == SPRITE
			...
			if (data->scene->map[i][j] == ENEMY)
				render_text(data, j, i, data->text_enemy);
			j++;
		}
		i++;
	}
...
int	render_init(t_data *data)
{
	...
	if (data->mlx_ptr == NULL)
		exit(exit_error(data, "MLX initialization failed"));
