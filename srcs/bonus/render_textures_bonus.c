...
/*   Updated: 2021/11/24 15:50:28 by jusaint-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	render_text(t_data *data, int x, int y, t_text *text)
{
	...
	while (i < CELL_WIDTH)
	{
		...
		while (j < CELL_WIDTH)
		{
			color = take_pix(j * text->width / CELL_WIDTH,
					i * text->height / CELL_WIDTH, text);
			img_pix_put(data->img, x * CELL_WIDTH + j,
				y * CELL_WIDTH + i, color);
			j++;
		}
		i++;
	}
...
int	texture_init(t_data *data, t_text *texture, char *path)
{
	...
	if (texture->mlx_img == NULL)
		exit(exit_error(data, "Failed to load texture"));
...
{
	texture_init(data, data->text_player, "srcs/textures/player-1.xpm");
	texture_init(data, data->text_player, "srcs/textures/player-2.xpm");
	...
	texture_init(data, data->text_sprite, "srcs/textures/collectible-2.xpm");
	texture_init(data, data->text_sprite, "srcs/textures/collectible-3.xpm");
